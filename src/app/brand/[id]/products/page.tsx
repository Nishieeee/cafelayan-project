...
import type { BrandProducts, Product } from "@/types/brand"
import { useAuth } from "@/context/AuthContext";

// Mock brand products data - would come from API
const brandProductsData: Record<string, BrandProducts> = {
  cafelayan: {
    name: "Cafelayan",
    products: [
      {
  ...
  dios: {
    ...
    products: [
      ...
      // },
      // {
      //   id: "ecobottle-seaweed-400",
      //   name: "EcoBottle Seaweed Wrap 400ml",
      //   brand: "EcoBottle Co",
      ...
      // },
      // {
      //   id: "ecobottle-mushroom-350",
      //   name: "EcoBottle Mushroom Base 350ml",
      //   brand: "EcoBottle Co",
...
export default function BrandProductsPage() {
  ...
  const [brandData, setBrandData] = useState<BrandProducts | null>(null)
  const { name } = useAuth();
  useEffect(() => {
    // Simulate API call
    const data = brandProductsData[brandId]
    if (data) {
      setBrandData(data)
    }
  }, [brandId])
  ...
  const filteredProducts = brandData.products.filter((product) => {
    const matchesSearch =
    ...
    <div className="container py-8 px-4 md:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">{brandData.name} - Product Management</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div data-aos="fade-up">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{brandData.products.length}</div>
              <p className="text-xs text-muted-foreground">
                {brandData.products.filter((p) => p.status === "Active").length} active
              </p>
            </CardContent>
          </Card>
          </div>
          ...
          <div data-aos="fade-up">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {brandData.products.reduce((sum, p) => sum + p.totalScans, 0).toLocaleString()}
              </div>
          ...
          <div data-aos="fade-up">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {brandData.products.reduce((sum, p) => sum + p.totalDonations, 0).toLocaleString()}
              </div>
          ...
          <div data-aos="fade-up">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {(
                  brandData.products.filter((p) => p.totalScans > 0).reduce((sum, p) => sum + p.conversionRate, 0) /
                  brandData.products.filter((p) => p.totalScans > 0).length
                ).toFixed(1)}
                %
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row gap-4">
              <div className="relative flex-grow">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 border-gray-500/50 h-4 w-4" />
                <Input
              ...
              <div className="flex gap-4">
                ...
                <Select value={materialFilter} onValueChange={setMaterialFilter}>
                  ...
                  <SelectContent className="bg-white"> 
                    ...
                    <SelectItem value="Biodegradable Plastic" className="hover:bg-green-300">Biodegradable Plastic</SelectItem>
                    <SelectItem value="Bamboo Fiber" className="hover:bg-green-300">Bamboo Fiber</SelectItem>
                    <SelectItem value="Corn Starch" className="hover:bg-green-300">Corn Starch</SelectItem>
                    <SelectItem value="Hemp Fiber" className="hover:bg-green-300">Hemp Fiber</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
        ...
        <div className="space-y-4">
          {filteredProducts.map((product) => (
            <Card data-aos="fade-up" key={product.id} className="overflow-hidden border-gray-500/50 hover:border-green-700">
        ...
        <AlertDialog open={!!deleteProductId} onOpenChange={() => setDeleteProductId(null)}>
          <AlertDialogContent className="border-gray-500/50">
            <AlertDialogHeader>
          ...
          <AlertDialog open={showQRDialog} onOpenChange={setShowQRDialog}>
            <AlertDialogContent className="max-w-md">
              ...
              <AlertDialogFooter className="flex-col sm:flex-row gap-2">
                <Button onClick={() => downloadQRCode(selectedProduct)} className="w-full sm:w-auto border-gray-500/50">
                  <Download className="mr-2 h-4 w-4" />
                  Download PNG
                </Button>
                <AlertDialogCancel className="w-full sm:w-auto border-gray-500/50">Close</AlertDialogCancel>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        )}
